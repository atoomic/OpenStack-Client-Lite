...

{
    ...
    qr/No API endpoint provided/,
      "OpenStack::Client->new() dies when no API endpoint URI is provided";
...

{
    ...
    foreach my $input (sort keys %tests) {
        ...
        is( $client->uri($input) => $expected,
            "\$client->uri() yields '$expected' for '$input'");
    }

    is( $client->endpoint => $endpoint,
        "\$client->endpoint() returns original HTTP endpoint URI");
    is( $client->token => $token,
        "\$client->token() returns original token object");
}

Test::OpenStack::Client->run_client_tests(
    {   'responses' => [{'content' => JSON::encode_json({'foo' => 'bar'})}],
    ...
    {
        'test' => sub {
            is( shift->call('GET' => '/foo') => '',
                "Empty response contains expected empty result");
        }
    },
    {
        'responses' => [
        ...
        'test' => sub {
            ...
            qr/420 Baked/,
              "\$client->call() dies when 400 error is returned by service";
        }
    },
    {
        'test' => sub {
            ...
            is_deeply(
                $got => $expected,
                "\$client decodes JSON response bodies");
        }
    },
    {
        'responses' => [
        ...
        'test' => sub {
            ...
            qr/Temporarily out of fucks to give/,
              "\$client->call() dies with response body when 400 or 500 status is raised";
        }
    },
    {
        'responses' => [
        ...
        'test' => sub {
            ...
            is( $got => $expected,
                "\$client->call() returns message body for plain text, non-error responses"
            );
        }
    },
    {
        ...
        'test' => sub {
            ...
            qr/504 Unknown error/,
              "\$client->call() dies with an unknown error when no response body is provided";
        }
    },
    {
        'test' => sub {
            ...
            my @tests = (
                ...
                ['/foo?bar=baz' => {'bar' => 'baz'}, "a single parameter"],
    ...
    },
    {
        'test' => sub {
            ...
            is( $got => $expected,
                "\$client->get() handles request paths with arguments already present"
            );
        }
    },
    {
        'responses' => [
            {   'content' =>
                  JSON::encode_json({'payload' => ['foo', 'bar', 'baz']})
            }
        ],
        ...
        'test' => sub {
            ...
            is_deeply(
                $got => $expected,
                "\$client->each() calls a callback for each paginated result");

            throws_ok {
                $client->each();
            }
            qr/Invalid number of arguments/,
              "\$client->each() dies when too few arguments are passed";

            throws_ok {
                $client->each('/foo', {}, sub { }, 'foo');
            }
            qr/Invalid number of arguments/,
              "\$client->each() dies when too many arguments are passed";
        }
    },
    {   'responses' => [{'content' => JSON::encode_json({})}],
        ...
        'test' => sub {
            ...
            is( $got => $expected,
                "\$client->each() handles GET parameters properly");
        }
    },
    {
        'responses' => [
            ...
            {'content' => JSON::encode_json({'items' => [{'meow' => 'cats'}]})}
        ],
        ...
        'test' => sub {
            ...
            is( scalar @responses => 3,
                "\$client->every() callback is called once per item across paginations"
            );
            ...
            is_deeply(
                \@responses => \@expected,
                "\$client->every() decodes response bodies as expected");
        }
    },
    {
        'responses' => [
            {   'content' =>
                  JSON::encode_json({'items' => ['foo', 'bar', 'baz']})
            }
        ],
        ...
        'test' => sub {
            ...
            is( $got => $expected,
                "\$client->every() passes along GET parameters fine");
        }
    },
    {   'responses' => [{'content' => '{}'}],
        ...
        'test' => sub {
            ...
            qr/Response from \/foo does not contain attribute/,
              "\$client->every() dies when responses lack attribute corresponding to result set";

            throws_ok {
                $client->every('/foo', 'items', sub { }, {}, 'bleh');
            }
            qr/Invalid number of arguments/,
              "\$client->every() will complain if argument count is too much";

            throws_ok {
                $client->every('/foo', 'items');
            }
            qr/Invalid number of arguments/,
              "\$client->every() will complain if argument count is too little";
        }
    },
    {
        'responses' => [
            ...
            {   'content' =>
                  JSON::encode_json({'items' => ['eins', 'zwei', 'drei']})
            }
        ],
        ...
        'test' => sub {
            ...
            is_deeply(
                \@got => \@expected,
                "\$client->all() handles result sets in the appropriate manner"
            );
        }
    },
    {   'responses' => [{'content' => JSON::encode_json({})}],
        ...
        'test' => sub {
            ...
            is $ua->{'requests'}->[0]->{'content'} => $sub,
              "\$client->call() passes CODE ref request body appropriately";
        }
    },
    {   'responses' => [{'content' => JSON::encode_json({})}],
    ...
    {   'responses' => [{'content' => JSON::encode_json({'items' => []})}],

        'test' => sub {
            my ($client, $ua) = @_;

            lives_ok {
                $client->all('/foo', 'items', {'meow' => 'cats'});
            }
            ...
            is( $got => $expected,
                "\$client->all() passes GET parameters on expectedly");
        }
    },
    {
        'responses' => [
            {'content' => JSON::encode_json({'bar'  => 'baz'})},
            {'content' => JSON::encode_json({'meow' => 'cats'})}
        ],
        ...
        'test' => sub {
            ...
            foreach my $test (@tests) {
                ...
                is( $got_method => $expected_method,
                    "\$client->$name() issues call with appropriate HTTP method"
                );
                is_deeply(
                    $got_response => $expected_response,
                    "\$client->$name() decodes response body appropriately");
    ...
    },
    {
        'test' => sub {
            ...
            lives_ok {
                $response =
                  $client->call('PATCH', $headers, '/foo', {'bar' => 'baz'});
            }
            ...
            is_deeply $req_headers => $headers,
              "4 argument form of \$client->call() sets headers as expected.";
